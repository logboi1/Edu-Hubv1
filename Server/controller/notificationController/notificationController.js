...
const privateVapidKey = 'I0_d0vnesxbBSUmlDdOKibGo6vEXRO-Vu88QlSlm5j0'

webpush.setVapidDetails(
  'mailto:ogundeleibraheem@gmail.com',
  publicVapidKey,
  privateVapidKey
)
...
const subscribe = async (req, res) => {
  try {
    ...
    const subscriptionData = await NotificationSubscription.findOne({
      user: user,
...
const pushNotification = async (user, payload, type = 'alert') => {
  try {
    ...
    subscriptions.forEach((subscription) => {
      ...
      webpush
        .sendNotification(subscription['subData'], payload)
        .then(() => {
